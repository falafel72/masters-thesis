...
coordinates $\mathfrak{p}$. This creates two rays that converge toward $p$: the
laser beam, and the ray from the camera that passes through $\mathfrak{p}$ on
the image plane. By projecting both of these rays outward until they intersect,
we can identify where $p$ is.
