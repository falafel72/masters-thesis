...
Viva Suresh, and Kyle Tran for contribution work described in this section.
* Testing
** Locations
...
During the Florida Keys deployment, individual fish were measured both with the
Fishsense Lite camera module, and a stereo Go Pro pair.

Even if the two devices produce different length measurements, statistical
methods can be used to adjust to known lengths if biases are consistent across
all measurements. One method used to measure the similarity of the measurements
between the two camera modules is the Kruskal-Wallis test. This measures the
...
measurements with different laser calibrations. Figure\nbsp\ref{} shows the results
from obtained from both calibrations - as can be seen, they are virtually
identical.

The reasoning for why may be that these two methods are theoretically
equivelant, in the sense that the line defined by the mean direction of
differences is in fact the optimal line that we get from our Gauss-Newton
formulation. Work to prove that still needs to be done.

This naturally gives us a much simpler method of performing laser calibration.

The author would like to acknowledge Professor Nikolay Atanasov for providing me
with the basis of this method.

**** Annotations :noexport:
include benchmarks maybe?
...
**** Laser Mount
<<sec:mount-failure>>
The laser mounts are made out of PLA, and therefore absorb
water. This causes the laser mounts to become brittle, and they have been shown
to break after a month or two of operation.
**** Laser Pointer
...
A collaboration with Backscatter
*** User Experience
