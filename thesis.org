...
Figure\nbsp\ref{fig:example-fish-picf}. One can imagine that as the fish gets further
...
not be considering passive data collection methods. More detail about existing
...
Our only unknown on the right hand side
...
of these conditions are violated, significant error is introduced.

A full field manual is provided in Appendix [[sec:user-manual]].
** Data Offloading
...
*** Nathan Hui's Pool
In times when Canyonview pool was unavailable, Nathan was kind enough for us to
lend us his pool for testing. I would like to thank Nathan for allowing us to do
so.
...
always underestimate Fred's length. There are several potential sources of error
...
errors in the distance to fish, and therefore the length of the overall fish.
...
Figure\nbsp\ref{fig:variances} shows the variances of the percentage measurement errors in all
...
These have typically failed after a month of use.
