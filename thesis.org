...
methods of gathering these data involve catch and release programs, which can
often harm the fish being measured, is expensive to coordinate, and only obtains
a limited sample of the fish population. These are also not possible in marine
...
laser pointer on a consumer waterproof camera, and obtain fish length purely
...
of laser parameters, as well as the calibration procedures from which this
knowledge is determined. We show that the accuracy of this solution is better
than visual estimation, to within 15% of the fish's true length.
...
potential impacts of overfishing and climate change.
...
of a fish population. One of our main collaborators, Prof. Brice Semmens, has
looked at the effects of fishing policies on a particular population of grouper,
and used an annual fish length distribution as the primary method of studying
their recovery\nbsp\cite{Heppell2012}. Fish length can also be used to convert to
mass, from which the biomass of a population can be calculated.

** Current Methods for Fish Length Studies
Current global fish length data typically come from catch and release programs.
In California, the California Collaborative Fisheries Research Program (CCFRP)
is one such program, which as of 2024, consists of over 1800 volunteer anglers.
These anglers typically use hook-and-line procedures to catch the fish, from
which length is measured directly with a tape measure before the fish is
released. This length information is combined with metrics like catch per unit
effort to estimate the biomass of a population. However, fishing takes time - in
their most recent study, each examined location took 3 days to gather data
...
anglers\nbsp\cite{Rohner2011}, and also the fish being studied\nbsp\cite{Ramsay2009, Campbell2010}.
Thus a low effort, non-invasive approach is desirable.
...
are often used to infer the abundance of a particular species.
...
#+CAPTION: An example fish image taken with the camera unit.
#+LABEL: fig:example-fish-pic
...
that in turn is used to get the fish length.

Our desired accuracy goal with this system is only within 15% of the true fish
...
errors. We aim for 15% as it improves upon the 20% accuracy from human-made
length estimates. Since our solution is constrained to citizen scientists, we
will only be comparing against other handheld systems and manual visual
estimation, and will not be considering passive data collection methods. More
detail about existing technologies is provided in
Section\nbsp[[sec:existing-technologies]].
...
** Conclusion
\\
So far, we have gone over the motivation and background for this project. In the
next chapter we will discuss the primary principles of the laser algorithm.
* Algorithm Details
<<sec:algorithms>>
** Preamble :ignore:
...
standard camera model\nbsp\cite{Agrawal2012}, hence we add a corrective optic in
order to correct these distortions.

Developed by Backscatter, this corrective optic is designed to attach to the
Olympus's underwater housing, and is typically used to widen the field of view
for underwater photographers. Future work is required for us to understand how
to model these Snell's law distortions better so we can ultimately remove it.

In order to verify that the pinhole model was sufficient for this camera system,
one metric we used was the mean squared error given by OpenCV's
...
pointers in order to gesture to others. Ideally, the laser is mounted rigidly to
the camera -- in practice, due to manufacturing defects and small perturbations,
the laser moves enough to cause significant error in the measurements. See
Section\nbsp[[sec:laser-calibration]] for how this issue is solved.
...
imaged by the camera, that this laser dot will have known pixel
coordinates $\mathfrak{p}$. This corresponds to a point $p_\text{image}$ on the
...
both $\vec{\ell p}$ and $\vec{O v}$ out until they intersect, we can identify where
$p$ is.

More specifically, we define scale factors $\lambda_1$ and $\lambda_2$ such that
\[
...
match up perfectly with where the ray $\vec{Ov}$ intersects with the image
plane. This causes the rays to come very close to each other, but not touch,
...
\[
x^{(k+1)} = x^{(k)} + (J_g^{T}J_g)^{-1}J_{g}\mathbf{g}(x)
\]
...
starting point is assumed to be $-4$cm in the x direction and $-11$cm in the $y$
...
developing our device we have suck to using the Hu method since the constraints
...
to obtain the length of a fish. In the next chapter, we will discuss the
procedures required to operate this device.
* Procedures
...
the restriction that the laser dot must be trained on the fish.

We expect two conditions in order to get an accurate measurement: we expect that
the diver be able to orient and maneuver themselves such that the fish is
perpendicular to the camera axis, and we expect the fish to be flat. If either
of these conditions are violated, significant error is introduced. A full field
manual is provided in Appendix [[sec:user-manual]].
** Data Offloading
...
** Conclusion :ignore:
\\
Herein we have summarized the procedures required to operate the FishSense Lite
camera module. The next chapter details how this camera module has been tested.
* Testing
** Locations
...
been used for these tests\footnote{If you would like to purchase a Weasley fish
of your own, go to https://www.loftus.com/items/LF-0167}. Fred is shown in
Figure \ref{fig:fred}.
...
To evaluate purely the distance estimates we can obtain from the system, we took
images of objects of known size, . These data were collected by taking pictures
at a known distance -- in our case, we taped 20cm increments to a long fishing
rod, and took images at each mark. There were originally 19 marks along the rod,
though many of the pieces of tape came off underwater. Results are shown in
Figure\nbsp\ref{fig:length-reference}, which demonstrates that the actual distance
and the measured distance are well correlated.

#+ATTR_LATEX: :scale 0.8
#+ATTR_ORG: :width 200px
#+CAPTION: Length estimates compared to a length reference.
...
always underestimate Fred's length. The primary sources of error have been due
to faulty calibration or issues with mount design. Previous datasets that show
error have been consistent, and according to
Equation\nbsp\ref{eq:length-from-depth}, the factors that can create a consistent
error include pixel pitch $w$ and focal length $f$.
...
higher than length measurements obtained in pool water, as shown in
Figure\nbsp\ref{fig:freshwater-vs-saltwater}. Since the cameras are calibrated in
fresh waterm, the change in refractive index in salt water is likely what causes
...
water, degradation from exposure to ultraviolet light, Current work is being
done to create these mounts out of a different material which does not absorb
water and resists corrosion.

The current design also puts significant stress on the parts that hold the laser
in place. Figure\nbsp\ref{fig:mount-crack} shows the most common stress point which
causes the mount to fail. This cracking is caused by the two screws used to keep
the laser secured to the mount.
...
**** Questions to ask REEF :noexport:
- how many dives did it take the laser mounts to break?
- how were the laser mounts being handled when they broke?
*** User Experience
...
Lite. We described main assumption -- the pinhole camera assumption, which
