...
\begin{equation*}
(\vb{d} R, \beta G, \gamma B)  \rightarrow \left(  \frac{  \vb{d} R  }{  \frac{\vb{d}}{n}  \sum_i R }, \frac{ \beta G }{  \frac{ \beta }{ n } \sum_i G} , \frac{  \gamma B  }{  \frac{ \gamma }{ n } \sum_i B } \right)
\end{equation*}
...
network (CNN) that is trained on $28\times28$ tiles within the image. When searching
...
networked called Fishial\nbsp\cite{Fishial2023} to segment the fish. This is a
MaskRCNN model that both identifies the species of the fish and draws a polygon
...
will be defined at the camera's optical center, where all light rays intersect.
This coordinate system is commonly known as the ``optical frame'', and any
mention of 3D coordinates in this work will be within this coordinate system.
...
We assume that this laser beam intersects the fish at an unknown point $\vb{p}$. As
imaged by the camera, this laser dot will have known pixel coordinates
$\mathfrak{p}$. This corresponds to a point $\vb{p}_\text{image}$ on the image plane,
defined as
...
We then define a parameter vector $\vb{x}$ that contains all of our parameters:
\[
\vb{x} = \begin{bmatrix}
\vb{d} \\
l_x \\
...
dot point $\vb{p}_i$, a straightforward method to get our laser trajectory $\vb{d}$ can be
obtained from a normalized sum over the differences between all points:
\[
...
images of objects of known length (in this case, a checkerboard square), at known
...
Notably, a change in the laser orientation $\vb{d}$ results in much more error than a
change in the laser location $l$, so error increases as the laser dot gets
...
Candelmo, Jen Loch, and Dylan Heppell for testing our system on behalf of REEF.
* Conclusion
<<sec:conclusion>>
** Summary
