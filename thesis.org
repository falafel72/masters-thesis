...
data, and to make th epractice more accessible to recreational divers. To
...
The pinhole camera model assumes that the image from a camera can be represented
on a plane in front of the optical center, called the image plane. This plane is
normal to the camera axis, and the distance between the focal point and this
plane is known as the focal length. This focal length is obtained from camera
calibration.
