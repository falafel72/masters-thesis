...
of estimating to within 20% of a fish's true length \cite{Harvey2001}. This skill also
requires retraining as frequent as once every 6 months \cite{Bell1985}.

This work presents a collaboration with the Semmens Lab and REEF: a device that
can be used by any recreational diver made from off-the-shelf components, that
produces estimations more accurately than a human being can. Dubbed Fishsense
Lite, this device incorporates laser triangulation techniques to allow divers to
take images of a fish and use them to estimate fish length.
...
that can cause major inaccuracies in depth estimation\nbsp
\cite{Agrawal2012,Luczynski2017}, though work on investigating solutions to
these problems is ongoing and is outside of the scope of this work.

In situations where higher accuracy is required, such as construction, this
particular device may not be ideal.
...
**** Notes :noexport:
Tie in Leica article, costs of underwater housings

something like: The cost of underwater equipment has decreased significantly
since the beginnings of scientific diving, which opens an entirely new avenue
for data collection. Underwater camera housings alone could cost upwards of
$2000 (adjusted for inflation), while the current proposed system only costs
about 1200.
...
instead be done with a dive slate -- something that most divers will carry. We
add pieces of duct tape in an arbitrary pattern on one side to make the slate
more featureful. We also make the assumption that the slate remains close to
...
*** Laser Calibration Algorithm Accuracy
*** Laser Attenuation Testing
...
units to be tested by REEF staff in the Florida Keys.
*** Annotations                                                    :noexport:
REEF helps Brice do population studies
Brice reached out to Fishsense to help develop camera, REEF provided funding through NOAA

I would like more context about the connection between REEF and the Semmens Lab.
These are questions I can ask when I meet them next week.

Specific list of questions:
- What drove REEF and the Semmens Lab to collaborate?
- What was the procedure? How often were these units used during deployment?
- What was the goal for deploying these units?
- Jen: Can I get that chi-squared graph?
