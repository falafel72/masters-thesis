...
<<sec:field-calibration-testing>> We have been using the checkerboard method as
the standard of calibration, as this can be easily automated. However, we want
to assess how effective slate calibration is in theory, as it would be a huge
asset to people using the device in the field. Luckily for us, the divers at
REEF were able to test the slate calibration with a slate of their own
construction. The methodology was as follows: each of their seven camera units
were calibrated using the same slate, and the length of the slate, show in
