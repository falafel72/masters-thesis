...
fish. These images can then be processed to obtain an approximation of the
fish's length.

** Pinhole Camera Model
All of the calculations shown assume that our camera follows the pinhole camera
assumption. This means that we assume that all light passes through a single
point known as the focal point. In air, this assumption is extremely common and
is used in stereo cameras for object triangulation. However, underwater it is
more difficult to make this assumption, especially with our current hardware
setup. Our camera's transparent port separating the camera lens from the water
is flat, meaning that Snell's law causes incoming light to refract. This
distorts images such that the projections from 3D space into the image plane are
no longer linear.

We currently address this issue by attaching a corrective optic to the camera,
and so far we have observed that this is sufficient to allow us to continue
making this assumption. Work is currently being done to attempt to model the
Snell's law distortions without the use of the corrective optic.
...
will be defined at the camera's optical center.

Given a particular fish image, we assume that we are able to trivially pick out
...
- [ ] pinhole camera model ("optical center")
- [ ] the image plane
...
simultaneously with a stereo Go Pro rig with a baseline of ??m. Figure\nbsp\ref{}
