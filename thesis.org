...
these data are time intensive, require expensive equipment and/or training.
Approaching this issue with citizen science -- utilizing efforts from the
general public -- allows us to gather much more data about fish populations that
would currently be costly to acquire. We are working towards developing a device
and software platform that allows recreational divers to take pictures of fish,
and have the images processed such that length and species information are
...
** Citizen Science
Our device seeks to take advantage of efforts from the general public in order
...
is otherwise only visible above water. We hope to both provide additional data
for fish specifically, as well as provide length data that can be analyzed.
...
#+CAPTION: Comparison of different underwater ranging techniques. The approach that FishSense Lite uses is in bold.
...
This system has been used to document the recovery of a population of Nassau
...
required to determine initial parameters, which is described in more detail in
Section\nbsp[[sec:laser-calibration]]. The color of the laser also has an effect,
...
internally, such that the raw images appear to be more distorted than the JPEGs
that the camera generates. In order to ensure that we have control over all
aspects of the image processing pipeline, we convert the raw camera images to a
PNG instead of working with JPEGs processed by the camera.

To convert from raw images to a PNG, we must first apply a debayering algorithm.
Pixels on an image sensor cannot inherently detect color, and colored filters
are applied in front of digital camera sensors so that color information can be
retrieved in post-processing. An illustration of this is included in
Figure\nbsp\ref{fig:bayer-pattern}.
...
displayed instead of the PNGs, as they are usually better looking.
...
the tile with the highest probability of being the laser dot is selected. We can
further constrain the tiles we need to feed into the network by also relying on
the laser parameters to select a smaller region of the image.
...
of the laser dot can be found in the 3D scene.
...
follow the pinhole camera model, especially with our current hardware setup.
Without the corrective optic mentioned in Section\nbsp[[sec:backscatter]], the flat port
of the camera housing causes the incoming light to refract.
...
In practice, these are hand-labeled. Automatic algorithms are currently being
developed, though these are outside the scope of this document.

Figure\nbsp\ref{fig:known-quantities} illustrates all the aforementioned quantities
in one place.
...
\[
        \alpha = \frac{\sum_{i=1}^n\sum_{j > i} p_j - p_i}{\lVert \sum_{i=1}^n\sum_{j > i} p_j - p_i \rVert}
\]

Once $\alpha$ is obtained, we take the centroid of all of our points, which
...
<<sec:fw-vs-sw>>
During the La Jolla deployment, we compared fake fish measurements between
