...
Python scripts that must be run sequentially, though
** Hardware
*** Overview :ignore:
...
camera. Distortions from Snell's law make this impossible using the standard
camera model\nbsp\ref{Agrawal2012}, so we add a corrective optic in order to correct
these distortions. Future work is required for us to understand how to model
these distortions better so we can ultimately remove this.
...
this. A comparison of the two methods is presented in Section\nbsp[[sec:math-testing]].
**** Hu
...
*** Raw Processing
Figure\nbsp\ref{fig:jpg-vs-png} shows a raw image compared with a JPEG from the same camera - we
can observe that the bottom of the pool in the raw image is slightly curved
whereas it is straight in the JPEG, implying that the camera does its own
...
images. The most important component of this stage is color correction, as
correcting the raw image in the right way will make the laser dot much easier to identify.
...
example of one of these is shown in Figure\nbsp\ref{fig:slate-calibration}. We add pieces of duct tape in
an arbitrary pattern on one side to make the slate more featureful. We also make
the assumption that the slate remains close to parallel with the image plane.
...
measurements of the duct tape pattern.
...
9. Use the detected laser point (as detailed in Section\nbsp[[sec:laser-detection]]) and project a ray out to the plane. The intersection is our calculated laser dot coordinate.
...
* Testing
** Locations
The data in this section were gathered from three locations, listed below.
*** Canyonview Pool
The vast majority of our data comes from Canyonview pool. The following is a
list of test dates along with a summary of what was done:
-
**** Note :noexport:
...
The Reef Environmental Education Foundation (REEF) is an organization affiliated with
the Semmens Lab. In August 2023, we deployed six of our Fishsense Lite
...
to measure fish lengths. These measurements are compared with each other in
Figure\nbsp\ref{}.
** Tests
*** Overall Accuracy
...
apparent size, which is depth dependent.

To test this, we measure a reference object. In order to fully simulate the
purpose for which the device was intended, we use a fake rainbow trout. Each
dummy is made from the same model, though different techniques have been used to
make them negatively buoyant. Three generations of dummy fish, named Fred,
George, and Ginny, have been used for these tests. Fred is shown in Figure
\ref{fig:fred}.
...
Figure\nbsp\ref{} shows the two laser dots side by side in clear water at a distance
of ??m. The red laser is barely visible, while the green laser is still very bright.
...
occurs is that fish are able to see green light far better than red.
...
orientation. We obtained the distance to the checkerboard using the position of
the laser dot as described in Section\nbsp\ref{sec:distance-determination}, and the
tilt angle of the board was extracted from its pose as described in
Section\nbsp\ref{sec:point-triangulation}. Using this data, from each image we
extracted 13 lengths, as shown in
...
#+ATTR_ORG: :width 200px
#+CAPTION: Percent error of all distances and angles, grouped by true object length.
...
**** Laser Mount
<<sec:mount-failure>>
The laser mounts are made out of PLA, and therefore absorb water.
**** Laser Pointer
...
2. A mechanical failure from the mechanism which activates the laser.
